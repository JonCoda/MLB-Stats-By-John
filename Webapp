            ...
            const getLiveScores = async () => {
                ...
                try {
                    ...
                    games.forEach(game => {
                        ...
                        const status = competition.status.type.detail; // Using 'detail' for more specific status
                        const teams = competition.competitors;
                        const home = teams.find(team => team.homeAway === 'home');
                        const away = teams.find(team => team.homeAway === 'away');

                        // Ensure scores are available before displaying, default to '0' if null
                        const awayScore = away.score !== null ? away.score : '0';
                        const homeScore = home.score !== null ? home.score : '0';

                        scoresHtml += `
                            <div class="score-item">
                                <strong>${away.team.displayName} @ ${home.team.displayName}</strong>
                                <span>Score: ${awayScore} - ${homeScore}</span>
                    ...
                    scoresDisplay.innerHTML = `<p class="text-center text-red-500">Error fetching scores: ${error.message}. Please check your internet connection.</p>`;
